...
contains
    ...
    subroutine plot()
      ...
      write(*,'(1x, 4f12.6)') minf, hinf, pinf, xmuinf
      ...
      do i = neta,1,-3
        ...
        if (i /= 1) then
          ...
          xm = sqrt((u(i)**2)/(0.4*t))
          pt2 = p(i) / pinf * (xm/minf)**7 * &
            ((7.0*minf**2-1.0)/(7.0*xm**2 - 1.0))**2.5
        end if
        !j = (pt2/3.0)*10+1 
        !ptg(j:j) = '*'
        write(*,'(1x,i2,5f12.6)') i, hinf, t, pinf, u(i), v(i)
        !write(*,'(1x,i2,6f12.6,2x,a11)') &
        !    i,r(i),u(i),v(i),p(i),t,xm,ptg
      end do
    end subroutine plot
end module prtplot
